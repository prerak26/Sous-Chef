  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password)) {
...
app.get('/auth', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password)) {
        res.status(200).send({ chefid: req.headers.userid });
  ...
  model.getChef(req.params.id)
    .then(async response => {
      if ((response.length !== 0)) {
        ...
        if (req.headers.userid === filteredResponse.chefId)
...
app.post('/recipe', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password))
        await model.getRecipeId()
          .then(async response => {
            recipeId = response;
            await model.createRecipe(recipeId, req.body.title, parseInt(req.body.serves),
              (req.body.isPublic ? 'public' : 'private'), req.headers.userid, req.body.duration)
              .then(async response => {
                await Promise.all([
  ...
  model.getRecipe(parseInt(req.params.id), req.headers.userid)
    .then(async response => {
      if ((response.length !== 0)) {
        ...
        if (reqRecipe.visibility === 'private' && req.headers.userid !== reqRecipe.authorid) {
        } else {
          await Promise.all([
    ...
    .finally(() => {
      ...
      } else if (reqRecipe.visibility === 'private' && req.headers.userid !== reqRecipe.authorid) {
...
app.post('/recipe/:id', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password))
        await model.getRecipe(parseInt(req.params.id))
          .then(async response => {
            if ((response.length !== 0)) {
              reqRecipe = response[0];
              if (reqRecipe.authorid !== req.headers.userid) {
                res.status(403).send({ message: "Unauthorized access" });
          ...
          .finally(() => {
            if (errorCaught !== null) {
              ...
              res.status(404).send({ message: "Recipe not found" });
              console.log(model.getDateTime(), 'GET: /recipe/:id', 404);
            } else if (reqRecipe.authorid === req.headers.userid) {
...
app.post('/recipe/shop/:id', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password))
        await model.getRequirementsByRecipe(parseInt(req.params.id))
          .then(async response =>
            await Promise.all(response.map(async (ingredient) => {
              await model.updateShoppingListIngredient(req.headers.userid, parseInt(ingredient.ingredientid), parseFloat(Number(ingredient.sum)))
                .catch(error => {
                  errorCaught = error;
                })
            }
  ...
  model.getRecipes(query_str)
    .then(async response => {
      await response.filter(async (recipe) => {
        return (recipe.visibility === 'public' || (recipe.visibility === 'private' && req.headers.userid === recipe.authorid));
      });
      res.status(200).send(response);
...
app.post('/bookmark/:id', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password)) {
        await model.createBookmark(req.headers.userid, parseInt(req.params.id))
          .then(response => {
...
app.delete('/bookmark/:id', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password)) {
        await model.removeBookmark(req.headers.userid, parseInt(req.params.id))
          .then(response => {
...
app.post('/rating/:id', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password)) {
        await model.rateRecipe(req.headers.userid, parseInt(req.params.id), req.body.rating)
          .then(response => {
...
app.delete('/rating/:id', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password)) {
        await model.unrateRecipe(req.headers.userid, parseInt(req.params.id))
          .then(response => {
...
app.get('/shoppinglist', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password))
        await model.getShoppingList(req.headers.userid)
          .then(response => {
            res.status(200).send(response);
...
app.post('/shoppinglist/:id', (req, res) => {
  ...
  model.getChef(req.headers.userid)
    .then(async response => {
      if ((response.length !== 0 && bcrypt.compareSync(req.headers.pswd, response[0].hashedpassword))
        || (req.headers.userid === process.env.user && req.headers.pswd === process.env.password))
        await model.deleteShoppingListIngredient(parseInt(req.params.id), req.headers.userid)
          .then(async deleteResponse => {
            await model.createShoppingListIngredient(parseInt(req.params.id), req.headers.userid, parseFloat(req.body.quantity))
